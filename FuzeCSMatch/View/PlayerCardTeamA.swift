    ...
    var body: some View {
        ZStack {
            HStack(alignment: .center, spacing: 0) {
                Spacer() // Espaçamento flexível
                
                VStack(alignment: .trailing, spacing: 0) {
                    Spacer() // Espaçamento para alinhar com o bottom da imagem
                    Text(playerNickName)
                        .font(.custom(Constants.CFRoboto.robotoMedium, size: 15))
                        .foregroundColor(Color.fromHex(Colors.textTitleColor.rawValue))
                        .lineLimit(2)
                        .padding(.bottom, 5) // Espaçamento entre nickname e name
                    Text(playerName)
                        .font(.custom(Constants.CFRoboto.robotoLight, size: 10))
                        .foregroundColor(Color.fromHex(Colors.textSubTileColor.rawValue))
                        .lineLimit(1)
                        .padding(.top, 5) // Espaçamento entre name e topo da célula
                }
                .frame(maxWidth: .infinity, alignment: .trailing) // Labels ocupam todo o espaço vertical
                ...
                Image(playerImage)
                    ...
                    .offset(y: -30) // Subir a imagem 15 pontos para alinhar com as labels
            }
            .padding(10)
            .frame(height: 100)
...
struct PlayerCardTeamB: View {
    ...
    var body: some View {
        ZStack {
            ...
            .frame(height: 100)
            .background(Color.fromHex(Colors.mainColor.rawValue))
            .cornerRadius(10) // Cantos arredondados da célula
            .edgesIgnoringSafeArea(.all)
        }
    }
